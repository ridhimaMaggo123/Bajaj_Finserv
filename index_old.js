...
const app = express();
// Parse PORT as integer, default to 3000 if not set
...
const getAIResponse = async (question) => {
  ...
  try {
    ...
    const prompt = `Respond with ONLY a single word or number to: ${question}`;
    const result = await model.generateContent(prompt);
    const response = await result.response;
    const text = response.text().trim();
    
    // Return only the first word/number
